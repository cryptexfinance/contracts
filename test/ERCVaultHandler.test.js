...
describe("ERC20 Vault", async function () {
    ...
    it("...shouldn't allow minting above cap", async () => {
        ...
        const reqAmount = await ercTokenHandler.requiredCollateral(amount);
        const mintFee = await ercTokenHandler.getMintFee(reqAmount);
    ...
    it("...should allow user to mint tokens", async () => {
        ...
        const mintFee = await ercTokenHandler.getMintFee(bigAmount);

        let tcapBalance = await tcapInstance.balanceOf(accounts[1]);
        expect(tcapBalance).to.eq(0);
        await expect(ercTokenHandler.connect(addr3).mint(amount, { value: mintFee })).to.be.revertedWith(
            "VaultHandler::vaultExists: no vault created"
        );
        await expect(ercTokenHandler.connect(addr1).mint(bigAmount, { value: mintFee })).to.be.revertedWith(
            "VaultHandler::mint: not enough collateral"
        );
        await expect(ercTokenHandler.connect(addr1).mint(0, { value: mintFee })).to.be.revertedWith(
            "VaultHandler::notZero: value can't be 0"
        );
        await expect(ercTokenHandler.connect(addr1).mint(amount, { value: mintFee }))
        ...
        await expect(ercTokenHandler.connect(addr1).mint(lowAmount, { value: mintFee })).to.be.revertedWith(
    ...
    it("...should calculate the burn fee", async () => {
        ...
        fee = await ercTokenHandler.getBurnFee(amount);
        expect(fee).to.eq(result);
    });
    ...
    it("...should allow users to burn tokens", async () => {
        ...
        const reqAmount2 = await ercTokenHandler.requiredCollateral(amount2);
        const ethAmount = await ercTokenHandler.getBurnFee(amount);
        const ethAmount2 = await ercTokenHandler.getBurnFee(bigAmount);
    ...
    it("...should test liquidation requirements", async () => {
        ...
        let amount = ethersProvider.utils.parseEther("10");
        const mintFee = await ercTokenHandler.getMintFee(amount);
        ...
        await ercTokenHandler.connect(lq).mint(amount, { value: mintFee });
    ...
    it("...should allow users to liquidate users on vault ratio less than ratio", async () => {
        ...
        const mintFee = await ercTokenHandler.getMintFee(liquidatorAmount);
        ...
        let vault = await ercTokenHandler.getVault(2);
        const burnAmount = await ercTokenHandler.getBurnFee(reqLiquidation);
        const aboveBurnAmount = await ercTokenHandler.getBurnFee(aboveReqLiquidation);
        const fakeBurn = await ercTokenHandler.getBurnFee(1);
    ...
    it("...shouldn't allow contract calls if contract is paused", async () => {
        ...
        await expect(ercTokenHandler.connect(addr1).mint(0, { value: 1 })).to.be.revertedWith("Pausable: paused");
        await expect(ercTokenHandler.connect(addr1).removeCollateral(0)).to.be.revertedWith(
            "Pausable: paused"
        );
    });
