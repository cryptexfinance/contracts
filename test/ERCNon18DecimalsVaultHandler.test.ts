...
export async function fakeDeployContract(
  name: string,
  deployer: Signer,
  args: any[]
) {
...
describe("ERC20 Vaults With Non 18 Decimal", async function () {
  ...
  beforeEach(async () => {
    ...
    CTX = await deployContract("Ctx", wallet, [
      wallet.address,
      wallet.address,
      now + 60 * 60,
    ]);
    orchestrator = await deployContract("Orchestrator", wallet, [
      wallet.address,
    ]);
    ...
    aggregatorInterfaceTCAP = await fakeDeployContract(
      "AggregatorInterface",
      wallet,
      []
    );
    ...
    aggregatorInterfaceWBTC = await fakeDeployContract(
      "AggregatorInterface",
      wallet,
      []
    );
    ...
    aggregatorInterfaceDAI = await fakeDeployContract(
      "AggregatorInterface",
      wallet,
      []
    );
    ...
    aggregatorInterfaceWETH = await fakeDeployContract(
      "AggregatorInterface",
      wallet,
      []
    );
  ...
  it("...should have same amount of collateral in USD", async () => {
    ...
    const wBTCCollateralRequired = await wBTCVaultHandler.requiredCollateral(
      tcapAmount
    );
    const DAICollateralRequired = await DAIVaultHandler.requiredCollateral(
      tcapAmount
    );
  ...
  it("...should have same vault ratio after burning TCAP", async () => {
    ...
    expect(oldBTCVaultRatio.toNumber()).to.be.closeTo(
      oldDAIVaultRatio.toNumber(),
      1
    );
    ...
    expect(newBTCVaultRatio.toNumber()).to.be.closeTo(
      newDAIVaultRatio.toNumber(),
      1
    );
  });
  ...
  it("...should have same liquidationReward when vault ratio goes down", async () => {
    ...
    const DAIRewardInUSD = newDAIOracleValue
      ...
      .div(BigNumber.from(`${10 ** 8}`))
      .div(BigNumber.from(`${10 ** 18}`));
    expect(BTCRewardInUSD.toNumber()).to.be.closeTo(
      DAIRewardInUSD.toNumber(),
      2
    );
  });
  ...
  it("...should have same vault ratio after liquidating vault", async () => {
    ...
    const DAIBurnFee = await DAIVaultHandler.getBurnFee(
      DAITcapLiquidationAmount
    );
    await wBTCVaultHandler.liquidateVault(1, BTCTcapLiquidationAmount, {
      value: BTCBurnFee,
    });
    await DAIVaultHandler.liquidateVault(1, DAITcapLiquidationAmount, {
      value: DAIBurnFee,
    });
  ...
  it("...should be able to liquidate when vault ratio falls below 100", async () => {
    ...
    const BTCVaultRatioAfterLiquidation = await wBTCVaultHandler.getVaultRatio(
      1
    );
