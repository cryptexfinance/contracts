...
describe("MATIC Vault", async function () {
	...
	it("...should allow user to mint tokens", async () => {
		...
		await expect(maticTokenHandler.connect(addr1).mint(bigAmount)).to.be.revertedWith(
		...
		await expect(maticTokenHandler.connect(addr1).mint(lowAmount)).to.be.revertedWith(
	...
	it("...should allow users to burn tokens", async () => {
		...
		const reqAmount2 = await maticTokenHandler.requiredCollateral(amount2);
		const ethAmount = await maticTokenHandler.getFee(amount);
		const ethAmount2 = await maticTokenHandler.getFee(bigAmount);
	...
	it("...should test liquidation requirements", async () => {
		...
		await maticTokenHandler.connect(lq).mint(amount);
	...
	it("...should allow users to liquidate users on vault ratio less than ratio", async () => {
		...
		const fakeBurn = await maticTokenHandler.getFee(1);
		await expect(
	...
	it("...shouldn't allow contract calls if contract is paused", async () => {
		...
		await expect(maticTokenHandler.connect(addr1).mint(0)).to.be.revertedWith("Pausable: paused");
		await expect(maticTokenHandler.connect(addr1).removeCollateral(0)).to.be.revertedWith(
			"Pausable: paused"
		);
	});
