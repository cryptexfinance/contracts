...
contract ArbitrumMessages is Test {
  ...
  function updateL1Relayer(address relayer) public {
    ...
    bytes memory callData = abi.encodeWithSelector(
      L2MessageExecutor.updateL2MessageRelayer.selector,
      relayer
    );
  ...
  function testUpdateL2MessageExecutor() public {
    vm.startPrank(user);
    assertEq(l1MessageRelayer.l2MessageExecutor(), address(l2MessageExecutor));
    l1MessageRelayer.updateL2MessageExecutor(user);
    assertEq(l1MessageRelayer.l2MessageExecutor(), user);
    vm.stopPrank();
  }

  function testRevertOnlyThisOnUpdateL2MessageRelayer() public {
    vm.expectRevert("L2MessageExecutor: Unauthorized message sender");
  ...
  function testUpdateL2MessageRelayer() public {
    assertEq(l2MessageExecutor.l1MessageRelayer(), user);
    updateL1Relayer(address(0x2));
    assertEq(l2MessageExecutor.l1MessageRelayer(), address(0x2));
  }
}
