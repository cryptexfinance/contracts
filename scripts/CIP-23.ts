...
async function main() {
  ...
  const l2MessageExecutorProxyAddress =
    "0x3769b6aA269995297a539BEd7a463105466733A5";
  ...
  const l2MessageExecutorProxy = _L2MessageExecutorProxy.attach(
  ...
  const TreasuryABI = [
    "function executeTransaction(address,uint256,string,bytes)",
  ];
  ...
  const _calldata = TreasuryIface.encodeFunctionData("executeTransaction", [
    ...
    abiCoder.encode(
      ["address", "uint256"],
      [ArbitrumMultisigAddress, ARBTokensToTransfer]
    ),
  ]);
  const payLoad = abiCoder.encode(
    ["address", "bytes"],
    [ArbitrumTreasuryAddress, _calldata]
  );
