// run with
// npx hardhat run ./scripts/CIP-15.ts --network hardhat
...
async function main() {
	...
	let ctxAmount = ethers.utils.parseEther("110000");
	let usdcAmout = ethers.utils.parseUnits("190000", 6);
	let ethAmount = ethers.utils.parseEther("21");
	...
	const description = "CIP-15: Q2 2022 Brand, Marketing, Operations & Development Budget";
	console.log(targets);
	console.log(values);
	...
	if (hardhatArguments.network === "hardhat") {
		...
		// Vote
		await castVote(8, true);

		// Wait to queue
		await queueProposal(8);

		// Execute transaction
		await executeProposal(8);
