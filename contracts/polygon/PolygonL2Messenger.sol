...
contract PolygonL2Messenger is IFxMessageProcessor, ReentrancyGuard {
  ...
  event FxRootSenderUpdate(
    address previousFxRootSender,
    address newFxRootSender
  );
  ...
  function processMessageFromRoot(
    ...
    address rootMessageSender,
    bytes calldata data
  ) external override nonReentrant onlyFxChild {
    require(
  ...
  function updateFxRootSender(address _fxRootSender) external onlyThis {
    require(
      _fxRootSender != address(0),
      "PolygonL2Messenger: _fxRootSender is the zero address"
    );
  ...
  function updateFxChild(address _fxChild) external onlyThis {
    require(
      _fxChild != address(0),
      "PolygonL2Messenger: _fxChild is the zero address"
    );
