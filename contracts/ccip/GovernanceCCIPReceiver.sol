...
contract GovernanceCCIPReceiver is CCIPReceiver {
     ...
     */
    constructor(address _router, uint64 _mainnetChainSelector, address _sender) CCIPReceiver(_router) {
        mainnetChainSelector = _mainnetChainSelector;
				mainnetSender = _sender;
    }

		/**
     * @notice Sets the address of the sender contract on Ethereum Mainnet.
     * @param _sender The address of the sender contract on Ethereum Mainnet.
     * @dev Only the contract itself can call this function.
     */
    function setMainnetSender(address _sender) external onlySelf {
      	emit MainnetSenderUpdated(mainnetSender, _sender);
				mainnetSender = _sender;
    }
    ...
    function _ccipReceive(Client.Any2EVMMessage memory message) internal override {
        ...
        if(messageSender != mainnetSender)
						revert Unauthorized(messageSender);
