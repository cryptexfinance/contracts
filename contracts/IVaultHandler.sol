...
/**
 * @title Vault Handler Abstract Contract
 * @author Cryptex.Finance
 * @notice Contract in charge of handling the Index Token and stake
 */
  ...
  IERC165
{
  ...
  /**
   * @notice Vault object created to manage the mint and burns of Index tokens
   * @param Id, unique identifier of the vault
   * @param Collateral, current collateral on vault
   * @param Debt, current amount of Index tokens minted
  ...
  /// @notice Value used as divisor with the value of the index token, just like the S&P 500 or any major financial index would to define the final token price
  uint256 public divisor;

  /// @notice Minimum ratio required to prevent liquidation of vault
  uint256 public ratio;

  /// @notice Fee percentage of the total amount to burn charged on ETH when burning Index Tokens
  uint256 public burnFee;

  /// @notice Fee percentage of the total amount to mint charged on ETH when burning Index Tokens
  ...
  /// @notice An event emitted when the minimum required Token is updated
  ...
  /**
   * @notice Sets the mint fee percentage an user pays when burning Index tokens
   * @param _mintFee uint
   * @dev Only owner can call it
   */
  ...
  /**
   * @notice Sets the burn fee percentage an user pays when burning Index tokens
   * @param _burnFee uint
   * @dev Only owner can call it
   */
  ...
  /**
   * @notice Sets the minimum token a vault owner has to mint
   * @param _minimumMint uint
   * @dev Only owner can call it
  ...
  function setMinimumMint(uint256 _minimumMint) external virtual onlyOwner {
    minimumMint = _minimumMint;
    emit NewMinimumMint(msg.sender, _minimumMint);
  }
  ...
  /**
   * @notice Allows users to remove collateral currently not being used to generate index tokens from their vaults
  ...
  /**
   * @notice Uses collateral to generate debt on index tokens which are minted and assigned to caller
    ...
    notZero(_amount)
  {
    ...
    require(
      vault.Debt >= minimumMint,
      "VaultHandler::mint: mint amount less than required"
    );
    indexToken.mint(msg.sender, _amount);
  ...
  /**
   * @notice Pays the debt of the Index token resulting them on burn, this releases collateral up to minimum vault ratio
  ...
  /**
   * @notice Allow users to burn Index tokens to liquidate vaults with vault collateral ratio under the minimum ratio, the liquidator receives the staked collateral of the liquidated vault at a premium
   * @param _vaultId to liquidate
   * @param _maxIndex max amount of Index the liquidator is willing to pay to liquidate vault
   * @dev Resulting ratio must be above or equal minimum ratio
   * @dev A fee of exactly burnFee must be sent as value on ETH
   * @dev The fee goes to the treasury contract
   */
  function liquidateVault(uint256 _vaultId, uint256 _maxIndex)
    external
    payable
    nonReentrant
    whenNotPaused
    whenNotDisabled(FunctionChoices.LiquidateVault)
  {
    ...
    uint256 requiredIndex = requiredLiquidationIndex(vault.Id);
    require(
      _maxIndex >= requiredIndex,
      "VaultHandler::liquidateVault: liquidation amount different than required"
    );

    uint256 fee = getBurnFee(requiredIndex);
    require(
      msg.value >= fee,
    ...
    //send back ETH above fee
    safeTransferETH(msg.sender, msg.value.sub(fee));
    emit VaultLiquidated(vault.Id, msg.sender, requiredIndex, reward);
  }
  ...
  /**
   * @notice Returns the price of the Index token
   * @return price of the Index Token
   * @dev Index token is 18 decimals
   ...
   * d = Divisor
   */
  function indexPrice() public view virtual returns (uint256 price) {
    uint256 totalMarketPrice = getOraclePrice(indexOracle);
    price = totalMarketPrice.div(divisor);
  }

  /**
   * @notice Returns the minimal required collateral to mint Index token
   * @param _amount uint amount to mint
   * @return collateral of the Index Token
   * @dev Index token is 18 decimals
    ...
    returns (uint256 collateral)
  {
    ...
    collateral = ((iPrice.mul(_amount).mul(ratio)).div(100))
      .div(collateralPrice)
      .div(collateralDecimalsAdjustmentFactor);
  }

  /**
   * @notice Returns the minimal required Index to liquidate a Vault
   * @param _vaultId of the vault to liquidate
   * @return amount required of the Index Token
   * @dev LT = ((((D * r) / 100) - cIndex) * 100) / (r - (p + 100))
   * cIndex = ((C * cdaf * cp) / P)
   * LT = Required Index
   * D = Vault Debt
   * C = Required Collateral
    ...
    returns (uint256 amount)
  {
    ...
    uint256 reqDividend = (
      ((vault.Debt.mul(ratio)).div(100)).sub(collateralIndex)
    ).mul(100);
    ...
    returns (uint256 rewardCollateral)
  {
    ...
    uint256 _rewardCollateral = (reward.mul(iPrice))
    ...
    returns (uint256 fee)
  {
    ...
    fee = (indexPrice().mul(_amount).mul(mintFee)).div(10000).div(ethPrice);
  }
   ...
   * 10000 = the value is 100 multiplied by 100 to support two decimals on the burn fee
   */
  function getBurnFee(uint256 _amount)
    public
    view
    virtual
    returns (uint256 fee)
  {
    uint256 ethPrice = getOraclePrice(ETHPriceOracle);
    fee = (indexPrice().mul(_amount).mul(burnFee)).div(10000).div(ethPrice);
  }

  /**
   * @notice Burns an amount of Index Tokens
  ...
  function _burn(uint256 _vaultId, uint256 _amount) internal {
    ...
    indexToken.burn(msg.sender, _amount);
  }
}
